...
public class AlipayPayOrderQueryService implements IPayOrderQueryService {
    ...
    public ChannelRetMsg query(PayOrder payOrder, MchAppConfigContext mchAppConfigContext){
        ...
        if(payOrder.getWayCode().equals(CS.PAY_WAY_CODE.ALI_FREEZE)){
            AlipayFundAuthOperationDetailQueryRequest req = new AlipayFundAuthOperationDetailQueryRequest();
            ...
            AlipayFundAuthOperationDetailQueryResponse resp = configContextQueryService.getAlipayClientWrapper(mchAppConfigContext).execute(req);
            String result = resp.getOrderStatus();

            if("AUTHORIZED".equals(result) || "FINISH".equals(result) || "CLOSED".equals(result)) {
                return ChannelRetMsg.confirmSuccess(resp.getOutOrderNo());  //资金已授权 或 已完成
            }else if("INIT".equals(result)) {
                return ChannelRetMsg.waiting(); //授权中
            }
            return ChannelRetMsg.waiting(); //授权中
        }else{
